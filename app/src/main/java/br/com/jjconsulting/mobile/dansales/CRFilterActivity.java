...
public class CRFilterActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (savedInstanceState != null && savedInstanceState.containsKey(KEY_FILTER_RESULT_STATE)) {
            mCRFilter = (CRFilter) savedInstanceState.getSerializable(
                    KEY_FILTER_RESULT_STATE);
        }

        if (getIntent().hasExtra(FILTER_RESULT_DATA_KEY)) {
            mCRFilter = (CRFilter) getIntent()
    ...
    protected void onSaveInstanceState(Bundle outState) {
        ...
        bundleFilter();
        outState.putSerializable(KEY_FILTER_RESULT_STATE, mCRFilter);
    }
    ...
    private void bundleFilter() {
        ...
        mCRFilter = new CRFilter(regiao, dateStart);
    }

    private void bundleEmptyFilter() {
        mCRFilter = new CRFilter();
    }

    private void sendBundledFilter() {
        Intent data = new Intent();
        data.putExtra(FILTER_RESULT_DATA_KEY, mCRFilter);
    ...
    private void setupRegiaoSpinner(String[] regioes) {
        ...
        if (mCRFilter != null && !TextUtils.isNullOrEmpty(mCRFilter.getRegiao())) {
            int indexOf = ArrayUtils.indexOf(objects, mCRFilter.getRegiao());
            if (indexOf > 0) {
    ...
    private void setupDataPicker() {
        ...
        if (mCRFilter != null) {
            if (mCRFilter.getDateStart() != null) {
                String date = FormatUtils.toDefaultDateFormat(mCRFilter.getDateStart());
                try {
